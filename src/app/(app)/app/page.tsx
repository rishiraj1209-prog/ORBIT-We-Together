    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <section className="rounded-[26px] border border-border/70 bg-surface/65 p-6 lg:col-span-2">
          ...
          <div className="mt-6 divide-y divide-border/60">{data.activities.length === 0 ? <div className="rounded-2xl border border-dashed border-border px-5 py-8 text-center"><p className="text-sm font-medium">Your live pulse starts here</p><p className="mt-2 text-xs leading-5 text-text-secondary">Real connections, messages, and introductions will appear as your alumni network grows.</p><Link href={APP_ROUTES.referrals} className="mt-4 inline-flex items-center gap-2 text-xs font-medium text-gold">Invite the first members <ArrowRight className="h-3 w-3"/></Link></div> : data.activities.slice(0,4).map((activity, i) => <Link key={activity.id} href={activity.href ?? "#"} className="group grid grid-cols-[32px_1fr_auto] items-center gap-3 py-4"><span className="font-mono text-[10px] text-text-tertiary">0{i+1}</span><span><span className="block text-sm font-medium">{activity.title}</span><span className="mt-1 block text-xs text-text-secondary">{activity.description}</span></span><ArrowRight className="h-4 w-4 text-text-tertiary transition-transform group-hover:translate-x-1 group-hover:text-gold"/></Link>)}</div>
        </section>
        <section className="rounded-[26px] border border-border/70 bg-surface/65 p-6"><p className="text-[10px] uppercase tracking-[.2em] text-text-tertiary">On your horizon</p><div className="mt-5 space-y-5">{data.upcomingEvents.length === 0 && data.opportunities.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-5"><p className="text-sm font-medium">The horizon is open</p><p className="mt-2 text-xs leading-5 text-text-secondary">Real events and member-posted opportunities will surface here.</p><div className="mt-4 flex gap-3"><Link href={APP_ROUTES.events} className="text-xs font-medium text-gold">Host event</Link><Link href={APP_ROUTES.opportunities} className="text-xs font-medium text-gold">Post role</Link></div></div> : <>{data.upcomingEvents.slice(0,2).map(event => <Link href={APP_ROUTES.events} key={event.id} className="block"><div className="flex gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent-subtle text-accent"><CalendarDays className="h-4 w-4"/></span><div><p className="text-sm font-medium">{event.title}</p><p className="mt-1 text-xs text-text-tertiary">{new Date(event.date).toLocaleDateString("en", { month: "short", day: "numeric" })} · {event.location}</p></div></div></Link>)}<div className="border-t border-border/60 pt-5">{data.opportunities.slice(0,1).map(opp => <Link href={APP_ROUTES.opportunities} key={opp.id} className="flex gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-ai-subtle text-ai"><Briefcase className="h-4 w-4"/></span><div><p className="text-sm font-medium">{opp.title}</p><p className="mt-1 text-xs text-text-tertiary">{opp.company} · Member posted</p></div></Link>)}</div></>}</div></section>
      </div>
    </div>
  );
}
